    ...
    <div className="admin-dashboard">
      ...
      <div className="dashboard-stats">
        <div 
          ...
          title="Click to view all mentors"
        >
          <h3>Total Mentors</h3>
        ...
        <div 
          ...
          onClick={() => {
            if (departments.length > 0) {
              toast.info(`Total ${departments.length} departments available`);
            } else {
              toast.warning("No departments found");
            }
          }}
          title="Click to view department count"
        >
          <h3>Total Departments</h3>
          <p className="stat-value">{departments.length}</p>
        </div>
        <div 
          className="stat-card pending-stat"
          onClick={() => {
            if (pendingMentors.length > 0) {
              window.scrollTo({ top: 0, behavior: 'smooth' });
              toast.info(`${pendingMentors.length} mentors awaiting approval`);
            } else {
              toast.success("No pending approvals!");
            }
          }}
